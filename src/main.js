...
app.whenReady().then(() => {
  ...
  // Si la app se abrió recién ahora *a partir de* un link de protocolo
  // (Windows la lanza con la URL como argumento), la procesamos apenas la
  // ventana esté lista.
  const launchLink = findProtocolLinkInArgv(process.argv);
  if (launchLink) {
    mainWindow.webContents.once('did-finish-load', () => handleProtocolLink(launchLink));
  }
